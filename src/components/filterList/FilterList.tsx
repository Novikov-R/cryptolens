...
const FilterList = () => {
	...
	return (
		...
		<tr>
			...
			<th className={`${thClass} cursor-pointer`}
				id="rank"
				onClick={() => dispatch(setActiveFilter('rank'))}>
			...
			<th className={`${thClass} cursor-pointer`}
				...
				<div className="flex justify-center items-center">Рыночная
					капитализация {renderSortIndicator('marketCapUsd')}</div>
			</th>
			<th className={`${thClass} cursor-pointer`}
				id="changePercent24Hr"
				onClick={() => dispatch(setActiveFilter('changePercent24Hr'))}>
				<div className="flex justify-center items-center">24ч % {renderSortIndicator('changePercent24Hr')}</div>
			</th>
		</tr>
		</thead>
	);
