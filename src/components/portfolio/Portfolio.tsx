...
const Portfolio = () => {
    ...
    formattedDif = dif === 0 ? '$0' : `${dif > 0 ? '+' : '-'} $${formattedDif}`;
        ...
        <>
            <Button variant={'ghost'} className='flex md:flex-row flex-col' onClick={() => setIsModalOpen(true)}>
                ...
                {!!cost && (
                    <div className='md:ml-1'>
                        {formattedDif} ({formattedDifInPercent} %)
                    </div>
                )}
            </Button>
            <Modal
                ...
                description={
                    <div>
                        Стоимость вашего портфеля: <span className='font-bold'>{formattedCost}</span>
                    </div>
                }
                footer={
            ...
            >
                ...
                {portfolioCoins.length ? (
                    <div className='space-y-3 mt-3 pt-3 border-t-[1px] border-gray-300'>
                    ...
                    <div className='text-center mt-2 text-sm text-gray-600'>Транзакции отсутствуют</div>
                )}
            </Modal>
        </>
    );
